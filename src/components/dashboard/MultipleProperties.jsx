...
const MultipleProperties = ({data,bids,loading,propertyCount}) => {
  ...
  const headersPropertyWinningAgent = ["Address", "Bids", "Winning Agent","Actions"];
    ...
    {loading?<Spin/>:<>
      <div className="bg-white  rounded-md w-full">
        ...
        const propertyBids = property?.bids;
          return (
         <>
         {  property?.winningAgent?
            <div className="w-full p-4 bg-white rounded-md overflow-x-auto border-b">
            <Table
              headers={headersPropertyWinningAgent}
              data={[[
                property?.address ? `${property?.address.street}, ${property?.address.city}` : "Address Missing",
                property?.bids ? property?.bids.length : "0",
                property?.winningAgent?.agency?.user?.name,
                <button onClick={() => navigate(`/property-details/${property._id}`, { state: { viewPage: 1 } })} className="py-1 px-3 text-custom_gray font-bold bg-spale_sunshine rounded uppercase">
                  sign agreement
                </button>,
              ]]}
              isDashboard={true}
         ...
         <div className="w-full p-4 bg-white rounded-md overflow-x-auto border-b">
           <Table
             ...
             data={[[
               property?.address ? `${property?.address.street}, ${property?.address.city}` : "Address Missing",
               property?.bids ? property?.bids.length : "0",
               moment( property?.endDateTime).format("DD/MM/YYYY"),
